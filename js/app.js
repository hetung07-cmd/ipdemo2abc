...
function buildIPsByDomain(items,order){
  ...
  var byTierName=function(a,b){
    return (tRank[ipTier(a)]||9)-(tRank[ipTier(b)]||9) || a.name.localeCompare(b.name);
  };
  var byDom={};
  ...
  domKeys.forEach(function(dom){
    ...
    Object.keys(bySub).sort().forEach(function(sg){
      ...
      bySub[sg].sort(byTierName).forEach(function(ip){
...
function showTT(id){
  ...
  if(f.deliverables&&f.deliverables.length){
    ...
    f.deliverables.forEach(function(x){
      if(typeof x==='string'){
        h+='<li class="del">'+E(x)+'</li>';
      }else if(x&&x.group){
        h+='<li class="del"><strong>'+E(x.group)+'</strong>';
        if(x.items&&x.items.length){
          h+='<ul class="del-sub">';
          x.items.forEach(function(it){h+='<li>'+E(it)+'</li>';});
          h+='</ul>';
        }
        h+='</li>';
      }
