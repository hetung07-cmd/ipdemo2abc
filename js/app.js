...
function citeMarkup(text){
  if(!text)return '';
  return E(text)
    .replace(/\[\[([a-z0-9-]+)\|([^\]]+)\]\]/g, function(_, id, label){
      return '<a href="more.html#'+id+'">'+label+'</a>';
    })
    .replace(/\[(\d+(?:\s*,\s*\d+)*)\]/g, function(_,n){
      return '<sup style="color:var(--spa-red);font-weight:700">['+n.replace(/\s+/g,'')+']</sup>';
    });
}
...
function showTT(id){
  ...
  /* 1. Description */
  h+='<div class="dblk"><div class="dblk-h">Description</div>'+citeMarkup(f.description||'')+'<p>'+figureHTML(f.image,f.image_caption)+'</p></div>';
  ...
  if(f.refs&&f.refs.length){
   ...
   /* 7. Read more — linked articles */
  if(f.read_more&&f.read_more.length){
    h+='<div class="dblk"><div class="dblk-h">Read more</div><ul>';
    f.read_more.forEach(function(r){h+='<li>'+citeMarkup(r)+'</li>';});
    h+='</ul></div>';
  }
  h+='</div>';
