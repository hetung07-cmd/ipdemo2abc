...
var GRP_BG={"N1": "#1A3C2E", "N2": "#2D5C46", "N3": "#0284C7", "N4": "#16A34A", "N5": "#065F46", "N6": "#1E3A5F", "N7": "#27272A", "N8": "#3F6212"};
var DOM_BG={"PDK": "#1A3C2E", "TD": "#2D5C46", "PILOT": "#0369A1", "HVM": "#166534", "METRO": "#065F46", "QR": "#1E3A5F", "MASK": "#374151", "PROD": "#5B21B6", "IT": "#78350F"};
var DOM_CARD={"PDK": {"bg": "#EBF1FB", "border": "#B8CCE8", "hdr": "#1F3D6F"}, "MASK": {"bg": "#FFF6E0", "border": "#E8D58C", "hdr": "#7A5A0E"}, "PROD": {"bg": "#FCE8E8", "border": "#EFC2C2", "hdr": "#7E2A2A"}, "TD": {"bg": "#E8F3E2", "border": "#BCD9A6", "hdr": "#33591A"}, "PILOT": {"bg": "#F1E6F7", "border": "#D2B5DE", "hdr": "#5C2470"}, "HVM": {"bg": "#FFEAD2", "border": "#E8C58C", "hdr": "#7C4416"}, "METRO": {"bg": "#DCEDE8", "border": "#A8CDC2", "hdr": "#1B4F46"}, "QR": {"bg": "#FAE0E0", "border": "#EDB6B6", "hdr": "#7A2424"}, "IT": {"bg": "#E3EAF2", "border": "#B0BFD2", "hdr": "#2A3F5C"}};
var PH_COL={"P0": {"bg": "#F8F9FF", "border": "#818CF8", "chip": "#4338CA", "txt": "#312E81"}, "P1a": {"bg": "#F0FDF4", "border": "#4ADE80", "chip": "#15803D", "txt": "#14532D"}, "P1b": {"bg": "#EFF6FF", "border": "#60A5FA", "chip": "#1D4ED8", "txt": "#1E3A8A"}, "P1c": {"bg": "#F0FDFA", "border": "#2DD4BF", "chip": "#0F766E", "txt": "#134E4A"}, "P2": {"bg": "#FFF7ED", "border": "#FB923C", "chip": "#C2410C", "txt": "#7C2D12"}, "P3": {"bg": "#F5F3FF", "border": "#C084FC", "chip": "#7E22CE", "txt": "#4C1D95"}};
...
function tBg(t){return {T1:'#C0392B',T2:'#B3D0C6',T3:'#347889',T4:'#fee2e2'}[t]||'#E2E8F0';}
function tFg(t){return {T1:'#fff',T2:'#1A3C2E',T3:'#fff',T4:'#991b1b'}[t]||'#374151';}
...
function buildIPsByDomain(items){
  if(!items||!items.length) return '<p style="color:#94A3B8;font-size:12px;font-style:italic">No IPs assigned.</p>';
  var byDom={};
  items.forEach(function(ip){(byDom[ip.domain]=byDom[ip.domain]||[]).push(ip);});
  var html='';
  Object.keys(byDom).sort().forEach(function(dom){
    var dIPs=byDom[dom],bg=DOM_BG[dom]||'#374151',emo=DOM_EMO[dom]||'';
    var bySub={};
...
function buildFlow(){
  ...
  h+='<div style="font-size:11px;font-weight:600;color:#C0392B;letter-spacing:.06em;text-transform:uppercase;writing-mode:vertical-rl;transform:rotate(180deg)">PARALLEL</div></div>';
...
function openPhaseModal(id){
  ...
  document.getElementById('phase-mhdr').style.color='#1A1A1A';
  ...
  document.getElementById('phase-mhdr').querySelector('.mhdr-close').style.color='#3D2B00';
  ...
  if(f.deliverables&&f.deliverables.length){
    ...
    c1+='<li><em style="color:#9B7355">No deliverables specified</em></li>';
  }
  c1+='</ul>';
  ...
  if(f.parallel) c2+='<div style="background:#eff6ff;border-left:3px solid #93c5fd;border-radius:0 5px 5px 0;padding:6px 10px;font-size:10.5px;color:#1d4ed8;margin-bottom:8px">&#9889; Runs in parallel with the other two P1 streams</div>';
  if(f.entry_criteria){
    c2+='<div class="pp-crit pp-entry">';
    c2+='<div class="pp-crit-title" style="color:#1D4ED8">&#128682; Entry Criteria</div>';
    c2+=criteriaList(f.entry_criteria);
    c2+='</div>';
  }
  if(f.exit_criteria){
    c2+='<div class="pp-crit pp-exit">';
    c2+='<div class="pp-crit-title" style="color:#16A34A">&#9989; Exit / Acceptance</div>';
  ...
  c3+='<span style="font-size:8.5px;color:#9B7355;font-style:italic">(tiers fixed at P0)</span></div>';
...
function renderMatrix(){
  ...
  /* Phase headers: #BA655A, white bold text, no chip background */
  ...
  html+='</tr></thead><tbody>';
  /* Group rows: #779976, white text, no badge background */
...
function openIPDetail(no){
  ...
  var bg=(DOM_CARD[dom]&&DOM_CARD[dom].hdr)||DOM_BG[dom]||'#374151';
  ...
  document.getElementById('ip-tier-badge').innerHTML=
    ...
    +'<span style="font-size:10px;color:#94A3B8;margin-left:8px;font-style:italic">(Tier fixed at P0 contract)</span>';
...
function buildIPMap(){
  ...
  LAYERS.forEach(function(layer){
    ...
    layer.domains.forEach(function(dom){
      var dIPs=byDom[dom]||[];
      var card=DOM_CARD[dom]||{bg:'#F8FAFC',border:'#E2E8F0',hdr:'#374151'};
...
function showDomainDetail(dom){
  ...
  if(ac) ac.style.outline='2px solid #C0392B';
}
...
function openFWModal(id){
  ...
  document.getElementById('fw-pp-hdr').style.background='linear-gradient(135deg,'+p.color+' 0%,#0F172A 100%)';
  ...
  c+='<div class="fw-pp-crit" style="border-left-color:#1D4ED8">';
  c+='<div class="fw-pp-crit-title" style="color:#1D4ED8">&#128682; Entry</div>';
  c+=criteriaList(p.entry_criteria)+'</div>';
  c+='<div class="fw-pp-crit" style="border-left-color:#16A34A">';
  c+='<div class="fw-pp-crit-title" style="color:#16A34A">&#9989; Exit</div>';
  ...
  Object.keys(byDomain).sort().forEach(function(dom){
    var card=DOM_CARD[dom]||{hdr:'#374151'};
  ...
  p.engineer_groups.forEach(function(grp){
    var grpColor=GRP_BG[grp[0]]||'#374151';
...
function openDTCOModal(id){
  ...
  body+='<div style="font-weight:700;color:#7E2A2A">Trigger:</div><div>'+E(loop.trigger)+'</div>';
  body+='<div style="font-weight:700;color:#7E2A2A">Cycle:</div><div>'+E(loop.cycle_time)+'</div>';
  body+='<div style="font-weight:700;color:#7E2A2A">From:</div><div>'+E(fromP?fromP.label:loop.from)+' ('+loop.from+')</div>';
  body+='<div style="font-weight:700;color:#7E2A2A">To:</div><div>'+E(toP?toP.label:loop.to)+' ('+loop.to+')</div>';
  body+='</div>';
...
function openExtModal(id){
  ...
  b+='<div style="background:#FAFAF8;border-radius:6px;padding:10px;margin-bottom:10px">';
  b+='<div style="font-weight:700;color:#7C4416;margin-bottom:4px;font-size:10px;text-transform:uppercase;letter-spacing:.04em">Enters Fab Work Flow at:</div>';
  s.enters_at.forEach(function(fwId){
    var fw=FW_MAP[fwId];
    b+='<span class="fw-tt-fw-chip" style="background:'+(fw?fw.color:'#374151')+';margin-right:4px">'+E(fwId)+' '+E(fw?fw.code:'')+'</span>';
  });
  b+='</div>';
  b+='<div style="background:#FAFAF8;border-radius:6px;padding:10px;margin-bottom:10px">';
  b+='<div style="font-weight:700;color:#7C4416;margin-bottom:4px;font-size:10px;text-transform:uppercase;letter-spacing:.04em">Example IPs in this stream:</div>';
  ...
  b+='</div>';
  b+='<div style="background:#FAFAF8;border-radius:6px;padding:10px;margin-bottom:10px">';
  b+='<div style="font-weight:700;color:#7C4416;margin-bottom:4px;font-size:10px;text-transform:uppercase;letter-spacing:.04em">Engineer groups involved:</div>';
  b+='<div style="font-size:10.5px">'+E(s.engineer_groups_involved.join(' &middot; '))+'</div>';
  b+='</div>';
  b+='<div style="font-size:10px;color:#7C4416;font-style:italic;border-top:1px dashed #E8D58C;padding-top:8px">&#9432; '+E(s.note)+'</div>';
